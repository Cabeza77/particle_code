...
i_image = 0
sigma = np.zeros( (N_R, N_theta) ) * u.cm/u.s # Initialize density array
try:
    sigma[:, :] = np.fromfile(data_dir+'/gasvrad'+repr(i_image)+'.dat').reshape(N_R, N_theta) * np.sqrt( aconst.G * aconst.M_sun / u.AU )
except:
...
sigma_min  = -5000.
sigma_max  = 5000.
levels     = np.linspace( sigma_min, sigma_max, 100 )      # Levels of colorbar
cbar_ticks = np.arange( sigma_min, sigma_max+0.1, 1000. )    # Ticks of colorbar
...
plot = ax.contourf(theta_cen, R_cen.to(u.AU), sigma[ :, :].cgs.value, cmap=amber_teal, levels=levels, extend='both' )       # Filled contour plot
...
for ires in resonances:
    plot_res.append( ax.plot(theta_cen, ires**(2./3.)*planet_R[i_image].to(u.AU)*np.ones_like(theta_cen), color="cyan", alpha=0.5) )
...
cbar.ax.set_ylabel(r' $v_\mathrm{rad}$  [ cm/s ]')
...
def update(val):
    ...
    try:
        sigma[:, :] = np.fromfile(data_dir+'/gasvrad'+repr(i)+'.dat').reshape(N_R, N_theta) * np.sqrt( aconst.G * aconst.M_sun / u.AU )
    except:
    ...
    dummy = ax.contourf(theta_cen, R_cen.to(u.AU), sigma[ :, :].cgs.value, cmap=amber_teal, levels=levels, extend='both' )
    for row in dummy.collections:
        collections_list.append(row)
    
    def format_coord_i(theta, r):
        value = intpol(theta, r, sigma[ :, :].cgs)
        return r'theta=%1.4f rad, R=%1.4f AU, Sigma=%1.4f cm/s' % (theta, r, value)
    ...
    # Update resonances
    for ires, res in enumerate(resonances):
        plot_res[ires][0].set_data( (theta_cen, res**(2./3.)*planet_R[i].to(u.AU)*np.ones_like(theta_cen)) )
