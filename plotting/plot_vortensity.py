...
i_image = 0
velocityunit = (u.au / u.year * 2. * const.pi).cgs
vrad   = np.zeros( (N_R, N_theta) ) * velocityunit
vtheta = np.zeros( (N_R, N_theta) ) * velocityunit
sigma = np.zeros( (N_R, N_theta) ) * aconst.M_sun / u.AU**2
try:
...
try:
    vrad[:, :] = np.fromfile(data_dir+'/gasvrad'+repr(i_image)+'.dat').reshape(N_R, N_theta) * velocityunit
except:
    print 'Could not load gasvrad'+repr(i_image)+'.dat'
try:
    vtheta[:, :] = np.fromfile(data_dir+'/gasvtheta'+repr(i_image)+'.dat').reshape(N_R, N_theta) * velocityunit
except:
...
##### COMPUTE VORTICITY ############################################################################################################
def computeVorticity(Ar, Atheta):
    R     = R_cen.cgs
    theta = theta_cen
    dArdtheta  = np.zeros( (N_R, N_theta) ) / u.s
    dRAthetadR = np.zeros( (N_R, N_theta) ) / u.s
    for it in range(N_theta):
        if(it>0 and it<N_theta-1):
            dArdtheta[:, it] = ( (Ar[:, it+1] - Ar[:, it-1]) / (theta[it+1] - theta[it-1]) ) / R[:]
    for iR in range(N_R):
        dArdtheta[iR, 0]  = ( (Atheta[iR, 1] - Atheta[iR, -1]) / (theta[1] - theta[-1]) ) / R[iR]
        dArdtheta[iR, -1] = ( (Atheta[iR, 0] - Atheta[iR, -2]) / (theta[0] - theta[-2]) ) / R[iR]
        if(iR>0 and iR<N_R-1):
            dRAthetadR[iR, :] = ( (R[iR+1]*Atheta[iR+1, :]-R[iR-1]*Atheta[iR-1, :]) / (R[iR+1]-R[iR-1]) ) / R[iR]
...
# Compute Vortensity
vort = computeVorticity(vrad, vtheta) / sigma

# Constrain the colorbar
vort_min   = np.min( vort[vort>0.].cgs.value )              # Minimum (non-empty) value of sigma
vort_max   = np.max( vort.cgs.value )                       # Maximum value of sigma
base       = 10.**( np.ceil( np.log10(vort_max) ) )
...
plot = ax.contourf(theta_cen, R_cen.to(u.AU), vort[ :, :].cgs.value, cmap=colormap, levels=levels, extend='both' )       # Filled contour plot
...
cbar.ax.set_ylabel(r'Vortensity  [ g/cm$^2/s$ ]')
...
def format_coord(theta, r):
    value = intpol(theta, r, vort[:, :].cgs)
    return r'theta=%1.4f rad, R=%1.4f AU, Vortensity=%1.4e g/cm2/s' % (theta, r, value)
...
def update(val):
    ...
    try:
        vrad[:, :] = np.fromfile(data_dir+'/gasvrad'+repr(i)+'.dat').reshape(N_R, N_theta) * velocityunit
    except:
        print 'Could not load gasvrad'+repr(i)+'.dat'
    try:
        vtheta[:, :] = np.fromfile(data_dir+'/gasvtheta'+repr(i)+'.dat').reshape(N_R, N_theta) * velocityunit
    except:
        print 'Could not load gasvtheta'+repr(i)+'.dat'
    # Compute vorticity
    vort = computeVorticity(vrad, vtheta) / sigma
    ...
    dummy = ax.contourf(theta_cen, R_cen.to(u.AU), vort[ :, :].cgs.value, cmap=colormap, levels=levels, extend='both')
...
def create_movie(event):
    ...
    for j, i in enumerate(np.arange(i_start, N_t+1)):
        ...
        movie_plot = movie_ax.contourf(theta_cen, R_cen.to(u.AU), vort[:, :].cgs.value, cmap=colormap, levels=levels, extend='both' )
        ...
        movie_cbar.ax.set_ylabel(r'Vortensity  [ g/cm$^2/s$ ]')
