...
cbar.ax.set_ylabel(r'Vorticity  [ 1/s ]')
...
def format_coord(theta, r):
    ...
    return r'theta=%1.4f rad, R=%1.4f AU, Vortisity=%1.4e 1/s' % (theta, r, value)
...
def update(val):
    ...
    def format_coord_i(theta, r):
        value = intpol(theta, r, vort[ :, :].cgs)
        return r'theta=%1.4f rad, R=%1.4f AU, Vorticity=%1.4e 1/s' % (theta, r, value)
...
def create_movie(event):
    ...
    for j, i in enumerate(np.arange(i_start, N_t+1)):
        ...
        movie_cbar.ax.set_ylabel(r'Vorticity  [ 1/s ]')
