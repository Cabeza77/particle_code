...
T_min  = np.min( T[T>0.].cgs.value )                                                      # Minimum (non-empty) value of sigma
T_max  = np.max( T.cgs.value )                                                            # Maximum value of sigma
levels     = np.linspace( np.floor(np.log10(T_min)), np.ceil(np.log10(T_max)), 100 )      # Levels of colorbar
cbar_ticks = np.arange( np.floor(np.log10(T_min)), np.ceil(np.log10(T_max))+0.1, 0.5 )    # Ticks of colorbar
...
plot = ax.contourf(theta_cen, R_cen.to(u.AU), np.log10(T[ :, :].cgs.value), cmap=colormap, levels=levels, extend='both' )       # Filled contour plot
...
def format_coord(theta, r):
    value = intpol(theta, r, T[ :, :].cgs)
    return r'theta=%1.4f rad, R=%1.4f AU, T=%1.4f K' % (theta, r, value)
...
def update(val):
    ...
    dummy = ax.contourf(theta_cen, R_cen.to(u.AU), np.log10(T[ :, :].cgs.value), cmap=colormap, levels=levels, extend='both' )
...
def create_movie(event):
    ...
    for j, i in enumerate(np.arange(i_start, N_t+1)):
        ...
        movie_plot = movie_ax.contourf(theta_cen, R_cen.to(u.AU), np.log10(T[:, :].cgs.value), cmap=colormap, levels=levels, extend='both' )
        ...
        movie_cbar.ax.set_ylabel(r'log T  [ K ]')
